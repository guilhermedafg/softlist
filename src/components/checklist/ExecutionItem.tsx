    ...
    >
      ...
      <div
        className={[
          ...
          concluido ? 'bg-success' : naoAplicavel ? 'bg-gray-300' : estaAtivo ? 'bg-black' : 'bg-transparent',
        ].join(' ')}
      />
      ...
      <div className="p-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <button
            type="button"
            onClick={aoToggle}
            aria-label={concluido ? 'Desmarcar item' : 'Marcar como concluído'}
            className={[
              'flex-shrink-0 w-11 h-11 rounded-full border-2 flex items-center justify-center',
              'transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2',
              'cursor-pointer select-none',
              concluido
                ? 'bg-success border-success text-white focus:ring-success animate-check-pop'
                : naoAplicavel
                ? 'bg-gray-200 border-gray-300 text-gray-400 focus:ring-gray-300'
                : 'border-gray-300 bg-white hover:border-black hover:bg-gray-50 focus:ring-black',
            ].join(' ')}
          >
            {concluido && (
          ...
          <div className="flex-1 min-w-0 pt-1">
            <h3
              className={[
                ...
                concluido ? 'line-through text-gray-400' : naoAplicavel ? 'line-through text-gray-400' : 'text-black',
              ].join(' ')}
            >
              {item.titulo}
            </h3>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {item.requerEvidencia && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full
                                 bg-caramel/10 border border-caramel/30 text-caramel
        ...
        {/* Descrição */}
        {item.descricao && (
          <p className={`mt-4 text-sm leading-relaxed pl-15 ${concluido ? 'text-gray-400' : 'text-gray-600'}`}
        ...
        <div className="mt-5" style={{ paddingLeft: '60px' }}>
          ...
          <textarea
            ...
            placeholder={concluido ? 'Adicione observações sobre este item...' : 'Descreva como foi a verificação, problemas encontrados...'}
            rows={2}
            className={[
        ...
        {/* Botão N/A */}
        <div className="mt-4 flex justify-end" style={{ paddingRight: '0' }}>
          <button
            type="button"
            onClick={() => {
              if (naoAplicavel) {
                aoToggle() // volta para pendente
              } else if (!concluido) {
                // toggle N/A — precisaria de um handler separado, aqui usamos aoToggle como proxy
                // O parent controla o ciclo: pendente → concluido → nao_aplicavel → pendente
              }
            }}
            className="text-xs text-gray-400 hover:text-gray-600 underline underline-offset-2 transition-colors"
          >
            {naoAplicavel ? 'Desfazer N/A' : 'Marcar como não aplicável'}
          </button>
        </div>
      </div>
    </div>
